...
void makePlots::Loop()
{
  ...
  TH2D* TwoPad_corr100=new TH2D("TwoPad_corr100","",9,0.5,9.5,1000000,0.,upbound);
   // TH2D* TwoPad_corr100=new TH2D("TwoPad_corr100","",9,0.5,9.5,1000000,0.,1.6);
  //
  //* Counters
  //
  int EventsPassed=0;
            ...
            if(iL == 1)
            {
                ...
                for (int ih = 0;ih < nhits;ih++)//outer hit loop
                {
                    for (int ij = ih+1; ij  < nhits;ij++)
                    {
                        ...
                        double dRing=0;
                        if(dRij>0.2 && (EMeV1*MEVTOGEV>2.*ENEPERMIP || EMeV2*MEVTOGEV>2.*ENEPERMIP)) {
