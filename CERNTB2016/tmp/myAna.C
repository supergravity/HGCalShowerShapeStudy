...
Bool_t myAna::Process(Long64_t entry)
{
  ...
  for (size_t tagIndex=0; tagIndex<4; tagIndex++)
	...
	const Float_t wave_max_Pad1 = wave_max[front];
	const Float_t pedrms_pad1 = pedestalRMS[front];
	...
	if (wave_max[front]/ADCperMIP[10*set+front] > nMIPs && pad!=front){
	  ...
	  */
	  ...
	  ///SJ added it
	  double sOvern = wave_max[pad]/pedestalRMS[pad];
	  double sOvern_ref = wave_max_Pad1/pedrms_pad1;
	  
	  double sOvern_12 = (sOvern * sOvern_ref)/sqrt( sOvern*sOvern + sOvern_ref*sOvern_ref ) ;


	  hists->fill2DHist(sOvern_12, t_max_frac50[pad] - refPad1,
			    "2D_Delay_from_Pad1_frac50_VS_SigOverNoise_"+suffix+tag,
			    ";Signal/Noise;T_{padX} - T_{pad1}, ns", 400, 2,160, 200, -0.6,0.6, 1,"Timing"+tag);
