...
///remember geometry in SIM is 90 degree rotated because the rows are clearly nicely arranged this way
///thre are 15 rows in coarse structure
///left cells is the number of cells in all in the previous rows

HexTopology::HexTopology(bool fine) {
  int cellCoarse[15] = {2,5,8,11,12,11,12,11,12,11,12,11,8,5,2}; //cells in a row
...
void HGCTB::Loop(std::string calFile, std::string wtFile, unsigned int nLayers,
  ...
  //TFile *f = TFile::Open("TBGenSim_1.root");
