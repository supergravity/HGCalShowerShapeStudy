...
void makeParametricModelDatacard(string infilename, string wsfilename="0", string outfilename="cms_hgg_datacard.txt", bool massfact=false, int mass=125){
  ...
  */
  ...
  string pathToDatBkgWS="CMS-HGG_new.root";
  //string pathToSigWS="CMS-HGG_sigfit.root";
  //string pathToSigWS="CMS-HGG_interpolated.root";
  string pathToSigWS="CMS-HGG.root";
  ...
  */
  ...
  TCut vbfloose("(category==36)");
  TCut vbftight("(category==37)");
  TCut allvbfcut("category==36 || category==37");
  TCut muon("(category==38)");
  TCut ele("(category==39)");
  TCut met("(category==40)");
...
//   catcuts.push_back(TCut("(bdtmod>=(-0.05) && bdtmod<0.49)"*!allvbfcut));
  ...
  cout<<"catcuts is : "<<catcuts[0]<<endl;
  cout<<"catcuts is : "<<catcuts[1]<<endl;
  cout<<"catcuts is : "<<catcuts[2]<<endl;
  */
  
  for (int icat=0; icat<36; icat++){

    char ccat[100];
    sprintf(ccat, "%d",icat);
    string scat(ccat);
    //catcuts.push_back(TCut( ("(category=="+scat+")").c_str() ) );
    catcuts.push_back( TCut(Form("category==%d",icat)) );
  ...
  if (massfact){
    ...
    */
    ...
    for (int icat=0; icat<36; icat++){
      ...
      sprintf(ccat, "%d",icat);
      string scat(ccat);
      //inccats.push_back(TCut( ("(inc_cat=="+scat+")").c_str() ) );
      inccats.push_back( TCut(Form("inc_cat==%d",icat)) ); 
  ...
  */
  ...
  fprintf(file, "shapes ggH * %s cms_hgg_workspace:roohist_sig_ggh_mass_m$MASS_$CHANNEL cms_hgg_workspace:roohist_sig_ggh_mass_m$MASS_$CHANNEL_$SYSTEMATIC01_sigma\n", pathToSigWS.c_str());
  fprintf(file, "shapes qqH * %s cms_hgg_workspace:roohist_sig_vbf_mass_m$MASS_$CHANNEL cms_hgg_workspace:roohist_sig_vbf_mass_m$MASS_$CHANNEL_$SYSTEMATIC01_sigma\n", pathToSigWS.c_str());
  fprintf(file, "shapes VH  * %s cms_hgg_workspace:roohist_sig_wzh_mass_m$MASS_$CHANNEL cms_hgg_workspace:roohist_sig_wzh_mass_m$MASS_$CHANNEL_$SYSTEMATIC01_sigma\n", pathToSigWS.c_str());
  fprintf(file, "shapes ttH * %s cms_hgg_workspace:roohist_sig_tth_mass_m$MASS_$CHANNEL cms_hgg_workspace:roohist_sig_tth_mass_m$MASS_$CHANNEL_$SYSTEMATIC01_sigma", pathToSigWS.c_str());
  ...
  for (unsigned int ieff=0; ieff<deffsinglesr.size(); ++ieff) {
	...
	/// SJ
	cout<<"icat:iproc:"<<icat<<":"<<iproc<<endl;
	/// SJ
	...
	/// SJ
	cout<<"ntot and hcount->GetSumOfWeights() is "<<hcount->GetSumOfWeights()<<":"<<ntot<<endl;
	cout<<"singlefrac is "<<singlefrac<<endl;
	/// SJ
	...
	/// SJ
	cout<<"singlefrac:"<<singlefrac<<endl;
	/// SJ
	...
	/// SJ
	cout<<"doublefrac:"<<doublefrac<<endl;
	/// SJ
